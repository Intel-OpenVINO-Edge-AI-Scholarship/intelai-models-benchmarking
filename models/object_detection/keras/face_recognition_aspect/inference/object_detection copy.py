...
def parse_args():
    ...
    parser.add_argument('--zoo', 
        ...
        help='An optional path to file with preprocessing parameters.', required=False)
    parser.add_argument('--input', help='Path to input image or video file. Skip this argument to capture frames from a camera.', required=False)
    parser.add_argument('--inputs', help='Path to input image or video file. Skip this argument to capture frames from a camera.', required=False)
    parser.add_argument('--out_tf_graph', default='graph.pbtxt',
        ...
        'argument. This way an additional .pbtxt file with TensorFlow graph will be created.', required=False)
    parser.add_argument('--framework', choices=['caffe', 'tensorflow', 'torch', 'darknet', 'dldt'],
        help='Optional name of an origin framework of the model. '
        'Detect it automatically if it does not set.', required=False)
    parser.add_argument('--thr', type=float, default=0.5, help='Confidence threshold', required=False)
    parser.add_argument('--nms', type=float, default=0.4, help='Non-maximum suppression threshold', required=False)
    parser.add_argument('--backend', choices=backends, default=cv.dnn.DNN_BACKEND_DEFAULT, type=int,
        help="Choose one of computation backends: "
        "%d: automatically (by default), "
        "%d: Halide language (http://halide-lang.org/), "
        "%d: Intel's Deep Learning Inference Engine (https://software.intel.com/openvino-toolkit), "
        "%d: OpenCV implementation" % backends, required=False)
    parser.add_argument('--target', choices=targets, default=cv.dnn.DNN_TARGET_CPU, type=int,
        help='Choose one of target computation devices: '
        '%d: CPU target (by default), '
        '%d: OpenCL, '
        '%d: OpenCL fp16 (half-float precision), '
        '%d: VPU' % targets, required=False)
    parser.add_argument('--async', type=int, default=0,
        help='Number of asynchronous forwards at the same time. '
        'Choose 0 for synchronous mode', required=False)
    parser.add_argument('--draw_pred', type=bool, default=False,
        help='Draw Predictions., Draw the predictions.', required=False)
    parser.add_argument('--motion_tracker', type=bool, default=False,
        help='Draw Predictions., Draw the predictions.', required=False)
...
def postprocess(frame, outs):
    ...
    global classIds, confidences, boxes
...
def callable(param_dict):
    ...
    # passing an input stream using ffmpeg
    if args.input:
